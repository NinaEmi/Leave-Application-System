...
public class DatabaseManager {
	...
	private void initializeDatabase() throws SQLException {
		try (Statement stmt = connection.createStatement()) {
			stmt.execute("CREATE TABLE IF NOT EXISTS employees (" + "id INTEGER PRIMARY KEY, " + "name TEXT, "
					+ "role TEXT, " + "team TEXT, " + "password TEXT, " + "vacationDays INTEGER)");

			stmt.execute("CREATE TABLE IF NOT EXISTS vacation_requests (" + "id INTEGER PRIMARY KEY AUTOINCREMENT, "
					+ "employeeId INTEGER, " + "startDate TEXT, " + "endDate TEXT, " + "reason TEXT, "
					+ "status TEXT)");
		}
	}
	...
	public Employee authenticateEmployee(int id, String password) {
		...
		try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
			...
			if (rs.next()) {
				return new Employee(rs.getInt("id"), rs.getString("name"), rs.getString("role"), rs.getString("team"),
						rs.getString("password"), rs.getInt("vacationDays"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public List<VacationRequest> getVacationRequestsForEmployee(int employeeId) {
		...
		try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
			...
			while (rs.next()) {
				VacationRequest request = new VacationRequest(rs.getInt("id"), rs.getInt("employeeId"),
	...
	public boolean hasOverlappingRequest(int employeeId, String startDate, String endDate) {
		 String sql = "SELECT * FROM vacation_requests WHERE employeeId = ? "
		            + "AND ((startDate <= ? AND endDate >= ?) "  // cerere existentă care acoperă noua perioadă
		            + "OR (startDate >= ? AND startDate <= ?)"   // cerere existentă care începe în noua perioadă 
		            + "OR (endDate >= ? AND endDate <= ?))"      // cerere existentă care se termină în noua perioadă
		            + "AND status != 'REJECTED'";
	...
	public void updateRequestStatusWithDeduction(VacationRequest request, RequestStatus status, String reason) {
	    String updateSql = "UPDATE vacation_requests SET status = ?, rejectionReason = ? WHERE id = ?";
	    try (PreparedStatement pstmt = connection.prepareStatement(updateSql)) {
	        pstmt.setString(1, status.name());
	...
	public List<String> getBlockedPeriods() {
		...
		try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
			while (rs.next()) {
				periods.add("Data perioadei blocate: " + rs.getString("startDate") + " - " + rs.getString("endDate")
	...
	public VacationRequest getVacationRequestById(int id) throws SQLException {
	    String query = "SELECT * FROM vacation_requests WHERE id = ?";
	    try (PreparedStatement stmt = connection.prepareStatement(query)) {
	        ...
	        try (ResultSet rs = stmt.executeQuery()) {
	            if (rs.next()) {
	                ...
	                // Crează obiectul VacationRequest
	                return new VacationRequest(id, employeeId, startDate, endDate, reason, status);
	            } else {
	                return null; // Dacă cererea nu este găsită
	            }
	        }
	    }
	}
